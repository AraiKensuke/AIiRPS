...
for blk in range(4):   #  the blocks here are relative to 'the_cnstrs'
    ...
    for partID in partIDs:
        ...
        netwins[ind, pid-1] = _N.sum(_hnd_dat[:, 2])

        wins = _N.where(_hnd_dat[0:-1, 2] == 1)[0]      #  HP wins
        ...
        loses = _N.where(_hnd_dat[0:-1, 2] == -1)[0]
        if len(loses) > 0:
        ...
        ties = _N.where(_hnd_dat[0:-1, 2] == 0)[0]            
...
for blk in range(4):
    ...
    for ca in cmp_againsts:
        ...
        pc, pv = rm_outliersCC(rout, biosig[ths])
...
# _plt.hist(cat, bins=bns, color="black")
# _plt.savefig("%(old)s_AQ_hist" % {"old" : oldnew})
