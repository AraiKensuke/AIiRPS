...
for datetm in datetms:
    ...
    for cov in [_AIconst._WTL]:#, _AIconst._HUMRPS, _AIconst._AIRPS]:
        ...
        if win_type == 1:
            ngs, ngsRPS, ngsDSURPS, ngsSTSW, all_tds, TGames  = empirical.empirical_NGS(datetm, win=wins, SHUF=SHUFFLES, flip_human_AI=flip_human_AI, expt=expt, visit=visit, dither_unobserved=False)
        elif win_type == 2:
            ngs, ngsRPS, ngsDSURPS, ngsSTSW, all_tds, TGames  = empirical.empirical_NGS_concat_conds(datetm, win=wins, SHUF=SHUFFLES, flip_human_AI=flip_human_AI, expt=expt, visit=visit)
