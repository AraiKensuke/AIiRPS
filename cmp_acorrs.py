...
for key in key_dats:
     ...
     for dat_mod in [[False, False], [True, False], [False, True], [True, True]]:
         ...
         pk_win = int(_N.ceil(pk_win_sec / (slideby/Fs)))
              ...
              if (maxs.shape[0] > 0) and (mins.shape[0] > 0):
                   ...
                   if (_N.sum(_N.isnan(mn_min_outer)) > 0) or (_N.sum(_N.isnan(mn_max_outer)) > 0):
                        pc = 0
                        maxy = 0
                        miny = 0
                   else:
                        #pc, pv = _ss.pearsonr(mn_min_outer, mn_max_outer)
                        pcL, pvL = _ss.pearsonr(mn_min[0:midp-wd], mn_max[0:midp-wd])
                        pcR, pvR = _ss.pearsonr(mn_min[midp+wd:], mn_max[midp+wd:])
         ...
         _plt.savefig("%(xd)s/cmp_xcorrs_show_%(ky)s_%(fr)s_%(lb)d%(rs)s" % {"xd" : cmp_xcorr_dir, "fr" : frg, "ky" : key, "lb"  : label, "rs" : srvrs})
         ...
         all_vecs = _N.zeros((sections, len(chs_picks), nStates))

         for epc in range(sections):
              all_vecs[:, :] = 0
         ...
         _plt.savefig("%(xd)s/cmp_xcorrs%(sw)s_%(ky)s_%(fr)s_%(lb)d%(rs)s" % {"xd" : cmp_xcorr_dir, "fr" : frg, "ky" : key, "rs" : srvrs, "lb" : label, "sw" : sweighted})
         _plt.close()
     
     #  stick image files together
     #for dat_mod in [[False, False], [True, False], [False, True], [True, True]]:
     fig = _plt.figure(figsize=(9, 10))
     ...
     for dat_mod in [[False, False], [True, False], [False, True], [True, True]]:
         ...
         fp = open("%(xd)s/cmp_xcorrs%(sw)s_%(ky)s_%(fr)s_%(lb)d%(rs)s.png" % {"xd" : cmp_xcorr_dir, "fr" : frg, "ky" : key, "rs" : srvrs, "lb" : label, "sw" : sweighted}, "rb")
     ...
     _plt.savefig("%(spd)s/cmp_4xcorrs%(sw)s_%(ky)s_%(fr)s_%(lb)d" % {"spd" : sub_prm_dir, "fr" : frg, "ky" : key, "lb" : label, "sw" : sweighted})
