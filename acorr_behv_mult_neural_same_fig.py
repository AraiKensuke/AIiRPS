...
#partIDs  = ["20200109_1504-32"]#"20210526_1416-25"]
...
partIDs  = ["20210526_1318-12", "20210526_1416-25",
            ...
            #"20200818_1644-09"]   #  1624_01 and 1416_25 are outliers
...
if fxdGK is not None:
    ...
    sMULT = "%.1f" % MULT
_fp = open("times_O_%(1)d_%(2)d_%(lags)d_%(lab)d_%(mult)s.txt" % {"1" : frng[0], "2" : frng[1], "lags" : lags_sec, "mult" : sMULT, "lab" : label}, "w")
#_fp_F = open("times_O_%(1)d_%(2)d_%(lags)d_%(mult)s_flip.txt" % {"1" : frng[0], "2" : frng[1], "lags" : lags_sec, "mult" : sMULT}, "w")
_fp_m_gi_ts = open("times_gmint_v_ACts_%(1)d_%(2)d_%(lags)d_%(lab)d_%(mult)s.txt" % {"1" : frng[0], "2" : frng[1], "lags" : lags_sec, "mult" : sMULT, "lab" : label}, "w")
...
for partID in partIDs:
    ...
    use_behv     = _N.array([_cnst._WTL])#, _cnst._AIRPS, _cnst._HUMRPS])
    ...
    for flip in [False, True]:
         fp = None if flip else _fp
         fp_m_gi_ts = None if flip else _fp_m_gi_ts 
         sflip = "" if not flip else "_flip"
        
         dNGS_timescales = _N.empty(sections)
         GCOH_timescales = _N.empty((nStates, sections))
        
         #bigchg_behv_sigs    = _N.array(_bigchg_behv_sigs)#[::-1] if flip else _bigchg_behv_sigs
         ...
         _cwtl_avg_behv_sig_interp = _N.interp(ts_gcoh, bigchg_behv_sig_ts/1000., _bigchg_behv_sigs)
         cwtl_avg_behv_sig_interp  = _N.array(_cwtl_avg_behv_sig_interp)
         cwtl_avg_behv_sig_interp[beginInd:endInd] = _cwtl_avg_behv_sig_interp[beginInd:endInd][::-1] if flip else cwtl_avg_behv_sig_interp[beginInd:endInd]
         ...
         for ns in range(nStates):
              ...
              for hlf in range(sections):
                   ...
                   if use_this[hlf, ns]:
                       ...
                       if flip == False:
                           _plt.plot(time_lags, dNGS_acs[hlf]+offset, color="red", lw=3)
                       else:
                           _plt.plot(time_lags, dNGS_acs[hlf]+offset, color="blue", lw=3)
                       _plt.plot(time_lags, fac01s[ns, hlf]+offset, color="black", lw=4)                       
         ...
         for h in range(sections):
             if dNGS_timescales[h] > 0:
                 ...
                 if len(mns) > 0:
                     #print(mns)
                     prs.append([_N.mean(mns) * (slideby/300), dNGS_timescales[h] * (slideby/300)])
         if not flip:
             for i in range(len(prs)):
                 fp.write("%(g).1f  %(C).1f\n" % {"g" : prs[i][0], "C" : prs[i][1]})
             fp.write("###########################%s\n" % partID)

         thsGCOH = _N.where(flat_GCOH_ts > 5)[0]
         ts_GCOH = _N.mean(flat_GCOH_ts[thsGCOH])
         thsdNGS = _N.where(flat_dNGS_ts > 5)[0]
    ...
    _plt.close()
    os.system("ln -sf %(od)s/f_acorr_behv_out_%(evn)d_%(w)d_%(sl)d_%(1)d_%(2)d_v%(av)d%(gv)d_%(lb)d.png  IMGLNK/f_acorr_behv_out_%(partID)s_%(evn)d_%(w)d_%(sl)d_%(1)d_%(2)d_v%(av)d%(gv)d_%(lb)d.png" % {"1" : fL, "2" : fH, "dat" : partID, "w" : win, "sl" : slideby, "av" : armv_ver, "gv" : gcoh_ver, "od" : outdir, "evn" : ev_n, "lb" : label, "partID" : partID})
